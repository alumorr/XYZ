...
const headers = {
  'User-Agent':'Mozilla/5.0 (iPhone; CPU iPhone OS 15_2 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Chrome/72.0.3626.121 Safari/537.36'
}
let login_token = '';
//需要修改的运动步数波动范围，脚本默认修改步数范围为1w9到2w5
const step = randomFriendPin($.getdata('xmMinStep')*1 || 25000, $.getdata('xmMaxStep')*1 || 45000);
function getToken() {
  if ($response.body) {
